...
fn run(params: &Params, emit: &mut Emit) {
    ...
    if !has_top_level {
        ...
        let stop = tree.roots.iter().position(|&r| r == first).unwrap_or(tree.roots.len().saturating_sub(1));
        has_top_level = tree
            .roots
            .iter()
            .take(stop)
            .all(|&r| NON_CONTENT_TOKENS.contains(&tree.get(r).kind) || is_html_flow_comment(tree, r));
    }
    if has_top_level {
